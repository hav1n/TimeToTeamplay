...
app.get('/Event/:page', function(request, response){
  ...
  var select = `<form action="/update_event/${page}" method="post">
  <select name="table">
...
app.post('/update_event/:page',function(request, response){
  ...
  response.redirect(`/Event/${eid}`)
})

app.get('/delete_event/:page',function(request,response){
  if(!authIsOwner(request, response)){
    response.redirect('/')
    return;
  }
